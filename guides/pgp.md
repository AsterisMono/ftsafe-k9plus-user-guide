...
此步导出的私钥含有：主密钥的私钥和**三个子密钥的私钥**。如果日后导入这个私钥，需要**重新执行“将子密钥导入智能卡”步骤**，否则**子密钥依然会保存在本地**。
...
> 这步究竟做了什么？
>
> 当执行keytocard命令时，gpg将子密钥的私钥汇入智能卡，并**删除本地此子密钥的私钥**，替换为一个**指向指定智能卡的指针**。
>
> 因此，当执行keytocard后，需要插入智能卡才能使用对应的加密/签名功能；
>
> 也因此，将全部的子密钥导入智能卡后，强烈推荐**再**备份一次私钥（此时私钥含有：主密钥的私钥，和三个子密钥的**指针**），方便在其他设备导入或恢复备份。
>
> 此时的私钥称为**被剥离的私钥**。请记住这个概念，后面还会用到。
